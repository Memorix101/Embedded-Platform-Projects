...
int main()
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_0; // LED on PA0
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = GPIO_PIN_1; // Button on PA1
    GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING; // Nothing -> High (Pull-Up), Pressed -> Low
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
    ...
    while (1)
    {
        ...
        if ((current_time - last_toggle_time >= 100) && !led_state) // Every 2 seconds
        {
